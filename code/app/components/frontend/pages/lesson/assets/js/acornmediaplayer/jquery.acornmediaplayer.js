...
(function($) {
	$.fn.acornMediaPlayer = function(options) {
		...
		var acornPlayer = function() {
			...
			var captionIndex = -1; // chung added
			...
			*/
			...
			// chung added
			var repeatCaption = function() {
				if (captionIndex != -1) {
					var start = captions[captionIndex].start;
					var end = captions[captionIndex].end;
					acorn.$self[0].currentTime = start;
					acorn.$self[0].play();
				}
			}
			...
			var init = function() {
				...
				// chung added
				acorn.$self.bind('repeat-caption', repeatCaption);
